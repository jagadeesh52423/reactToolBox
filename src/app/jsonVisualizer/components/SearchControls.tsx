            ...
            <div className="flex items-center gap-3">
                ...
                <div className="flex-1 relative">
                    ...
                    <input
                        ...
                        className="w-full pl-10 pr-10 py-2.5 bg-slate-900/50 border border-slate-600/50 rounded-lg text-slate-200 text-sm placeholder-slate-500 focus:outline-none focus:border-indigo-500/50 focus:ring-1 focus:ring-indigo-500/25 transition-all"
                    />
                    {searchText && (
                        <button
                            onClick={handleClear}
                ...
                <div className="relative">
                    ...
                    <select
                        ...
                        className="appearance-none pl-8 pr-8 py-2.5 bg-slate-900/50 border border-slate-600/50 rounded-lg text-slate-300 text-sm focus:outline-none focus:border-indigo-500/50 cursor-pointer"
            ...
            <div className="flex items-center justify-between mt-3">
                <div className="flex items-center gap-4">
                    ...
                    <label className="flex items-center gap-2 cursor-pointer group">
                        <div
                            className={`relative w-9 h-5 rounded-full transition-colors duration-200 ${
                                isFuzzyEnabled ? 'bg-purple-600' : 'bg-slate-600'
                            }`}
                    ...
                    <label className="flex items-center gap-2 cursor-pointer group">
                        <div
                            className={`relative w-9 h-5 rounded-full transition-colors duration-200 ${
                                isFilterEnabled ? 'bg-blue-600' : 'bg-slate-600'
                            }`}
                        ...
                        <FilterIcon size={14} className={isFilterEnabled ? 'text-blue-400' : 'text-slate-500'} />
                        <span className="text-sm text-slate-400 group-hover:text-slate-300 transition-colors">
                            Filter results
                        </span>
                    </label>
                ...
                {searchText && matchCount !== undefined && (
                    <div className="text-sm text-slate-500">
                        <span className="text-slate-300 font-medium">{matchCount}</span>
