...
const NodeStylePanel: React.FC<NodeStylePanelProps> = ({
  ...
  }, [activeStyleProperty, selectedNodes, nodeGradients]);
  ...
  return (
    <div>
      ...
      <div ref={toolbarRef} className="sticky top-0 z-10 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-600 pb-3 -mx-4 px-4 pt-0">
        ...
        {/* Color picker with Solid/Gradient tabs */}
        {mounted && isColorPicker && (
          <div className="mt-3 bg-gray-50 dark:bg-slate-800 border-t border-gray-200 dark:border-slate-600 rounded-b-lg p-3">
            ...
            <div className="flex mb-3 border-b border-gray-200 dark:border-slate-600">
              <button
                className={`px-3 py-1 text-sm font-medium ${colorMode === 'solid' ? 'border-b-2 border-blue-500 dark:border-blue-400 text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-300'}`}
                onClick={() => setColorMode('solid')}
              >Solid</button>
              <button
                className={`px-3 py-1 text-sm font-medium flex items-center gap-1.5 ${colorMode === 'gradient' ? 'border-b-2 border-blue-500 dark:border-blue-400 text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-slate-400 hover:text-gray-700 dark:hover:text-slate-300'}`}
                onClick={() => setColorMode('gradient')}
              >
                <div className="w-3 h-3 rounded-full" style={{ background: 'linear-gradient(to right, #60a5fa, #a78bfa)' }} />
                Gradient
              </button>
            </div>
            ...
            {colorMode === 'solid' ? (
              ...
              <>
                ...
                {/* Color stops with inline pickers */}
                <div className="space-y-2 mb-3">
                  {gradientStops.map((stop, index) => (
                    <div key={index} className="flex items-center gap-2">
                      <span className="text-xs text-gray-500 dark:text-slate-400 w-6 flex-shrink-0">{stop.offset}</span>
                      <input type="color" value={stop.color} onChange={(e) => updateStopColor(index, e.target.value)}
                        className="w-8 h-8 rounded border border-gray-300 dark:border-slate-600 cursor-pointer flex-shrink-0" />
                      <span className="text-xs text-gray-500 dark:text-slate-400 font-mono flex-1">{stop.color}</span>
                      {gradientStops.length > 2 && (
                        <button onClick={() => removeGradientStop(index)}
                          className="text-xs text-red-500 dark:text-red-400 hover:text-red-700 dark:hover:text-red-300 px-1" title="Remove stop">
                          ✕
                        </button>
                      )}
                    </div>
                ...
                <div className="flex items-center gap-2">
                  {gradientStops.length < 5 && (
                    <button onClick={addGradientStop}
                      className="text-xs px-2 py-1 rounded border border-gray-300 dark:border-slate-600 text-gray-700 dark:text-slate-300 hover:bg-gray-100 dark:hover:bg-slate-700">
                      + Add Stop
                    </button>
                  )}
